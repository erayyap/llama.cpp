...
The Q8 DSpark already includes a trained per-position confidence head exposed by `--spec-draft-p-min`, so it was evaluated as the zero-extra-target-compute alternative. Confidence predicted per-position acceptance well (AUC 0.846 across 1,572 labeled positions from 463 blocks), but a corrected 0.40 cutoff improved the unweighted three-workload ABBA mean by only 0.50%: arithmetic +0.96%, prose +0.79%, coding -0.27%. Control and cutoff output hashes diverged on prose and coding, so the path was numerically and behaviorally distinct despite exact final target verification. More aggressive 0.50/0.60 cutoffs regressed completed workloads. All instrumentation and controller experiments were removed; no partial-target implementation was retained.
...
The retained rows4 projection cuts `q8_0 m=32768,n=1,k=1024` by roughly 45%. Reusing its already-generated width-specific pipelines for `n=2..5` passed focused Vulkan operation checks but was neutral: -0.11%, +0.14%, -0.08%, and -0.70% respectively. The generic multi-query path already amortizes the weight traffic, so the production selector remains width one only.

Raw operation ABBA: `/home/canavar/benchmarks/dsv4-inference-research/qb-multiquery-rows4/`.

### Semantic MoE expert pruning

A runtime experiment retained all six experts for the three hash-routed layers and reduced learned-router layers from top six to top four or five. Top four produced a large 12.00% unweighted three-workload ABBA-mean gain, but failed the fixed arithmetic gate with `924` instead of `1901`; it was rejected immediately. Top five passed all 10/10 semantic short validators and long retrieval, and improved the unweighted three-workload ABBA mean by 5.00%, but prose regressed 6.19% and WikiText-2 512 perplexity degraded 5.84%, from the archived control 4.9479 to 5.2370. That is meaningful model-quality loss, so top five was also rejected. No expert-pruning code or runtime switch was retained.
